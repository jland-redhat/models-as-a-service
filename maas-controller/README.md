...
- Writes merged **`deployment/base/maas-controller/default/params.env`** (operator path) by merging, in order: the on-disk template, `RELATED_IMAGE_*` for any image keys still empty, the live **`maas-parameters`** `ConfigMap`, then Tenant-driven keys. Renders **`maas-api/deploy/overlays/odh`**. It **does not apply** the generated **`maas-parameters`** manifest (operator-owned). **Post-render** reads the live **`maas-parameters`** `ConfigMap` again and sets **container images** on `maas-api`, `payload-processing`, and `maas-api-key-cleanup` so digests match the operator even when kustomize used older `params.env` defaults. **`maas-api`** env for gateway and API key TTL uses **`maas-parameters`** via shared patches; **`maas-controller`** reads gateway and cache TTL keys from **`maas-parameters`** in **`deployment/base/maas-controller/manager/manager.yaml`** (ship a new bundle or re-apply controller manifests for that change).
